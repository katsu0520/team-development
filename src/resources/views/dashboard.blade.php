        ...
        <div class="flex flex-col h-full w-full overflow-hidden">
            ...
            <section class="flex w-full">
                ...
                <section class="w-80">
                    ...
                    <section class="bg-white rounded-3xl p-3 shadow-xl">
                        <p class="font-zenkaku font-bold mx-2">主要16項目・満足度</p>
                ...
                <section class="flex-1 px-8 py-3 bg-customLightPink rounded-3xl text-white ml-4 shadow-xl">
                    ...
                    <div class="my-1 flex h-1/2">
                        <div>
                            <h2 class="text-3xl font-bold font-zenkaku">主要<span class="font-mono">16</span>項目・満足度</h2>
                    ...
                    <div class="my-1 flex h-1/2">
                        <div>
                            <div class="flex">
                                <h2 class="text-3xl font-bold font-zenkaku">期待度 × 満足度</h2>
            ...
            <section class="flex w-full mt-2">
                ...
                <section class="flex flex-col w-1/2 ml-3 space-y-2">
                    <!-- 満足度ステータス -->
                    <div class="h-1/3 shadow-xl rounded-3xl py-1.5 {{ $scoreDifference > 0 ? 'bg-customBlue bg-opacity-50' : 'bg-customYellow bg-opacity-50' }}">
                        <p class="font-bold flex items-center px-6 {{ $scoreDifference > 0 ? 'text-teal-700' : 'text-yellow-600' }}">
                            全社主要16項目・満足度
                        </p>
                        <div class="border-t {{ $scoreDifference > 0 ? 'border-teal-600' : 'border-yellow-500' }} mt-0.5 mb-1"></div>
                        <p class="mt-0.5 font-bold px-6 {{ $scoreDifference > 0 ? 'text-teal-700' : 'text-yellow-600' }}">
                    ...
                    <!-- ボトルネックステータス -->
                    <div class="h-1/3 shadow-xl rounded-3xl py-1.5 {{ $bottleneckDifference > 0 ? 'bg-customYellow bg-opacity-50' : 'bg-customBlue bg-opacity-50' }}">
                        <p class="font-bold flex items-center px-6 {{ $bottleneckDifference > 0 ? 'text-yellow-600' : 'text-teal-700' }}">
                            全社期待度×満足度ボトルネックスコア
                        </p>
                        <div class="border-t {{ $bottleneckDifference > 0 ? 'border-yellow-500' : 'border-teal-600' }} mt-0.5 mb-1"></div>
                        <p class="mt-0.5 font-bold px-6 {{ $bottleneckDifference > 0 ? 'text-yellow-600' : 'text-teal-700' }}">
                            @if($bottleneckDifference > 0)
                                要注意です
                            @else
                                改善傾向です
                            @endif
                    ...
                    <div class="h-1/3 shadow-xl rounded-3xl bg-white py-1.5">
                        ...
                        <div class="flex px-6">
                            ...
                            <div class="ml-auto">
                                <a href="{{ route('analytics', ['dept_id' => $currentDeptId]) }}" class="flex items-center justify-center hover:bg-opacity-80 transition">
