    ...
    <div class="flex gap-x-3">
        <section class="flex flex-col w-2/3 gap-y-3">
            ...
            <div class="flex bg-white rounded-3xl px-8 py-4 border-4 border-customPink" id="top">
                        ...
                        <div class="flex items-center px-2 w-full">
                            <div class="mx-4 my-6 w-full">
                                ...
                                <div class="relative w-[500px] h-6 mt-2 rounded-full overflow-hidden bg-gray-300">
                                    <div id="latest-bar" class="absolute h-full bg-customBlue opacity-70 rounded-full z-0" style="width: 29%;" value="29"></div>
                                    <input
                                    id="satisfaction"
                                    type="range"
                                    min="0"
                                ...
                                <p class="text-customBlue text-lg mt-2 font-bold">
                                    最新の満足度 <span id="latest-val">29%</span>
                                </p>
                            </div>
                            <script>
                ...
                <div class="w-1/5">
                    <div class="flex items-center">
                        <input type="date" class="text-gray-500 px-2 py-1 border-none outline-none focus:outline-none focus:ring-0 cursor-pointer">
                    </div>
            ...
            <div class="flex bg-white rounded-3xl px-8 py-4 border-4 border-customPink">
                <div class="flex flex-col w-4/5 text-customNavy">
                    ...
                    <div id="accordionContent" class="transition-all duration-500 overflow-hidden">
                        ...
                        <div class="border-b border-gray-300 my-1"></div>
                        <button class="bg-customPink flex text-white px-4 py-4 rounded-xl mt-2 w-full planning-btn" data-url="{{ route('planning', ['dept_id' => $currentDeptId]) }}">
                ...
                @foreach ($items as $index => $item)
                    ...
                    @endif
                        <div class="flex flex-col w-4/5 text-gray-500">
                            <div class="flex items-center">
                                <button id="accordionToggle{{ $index }}" class="relative group mr-2 accordion-toggle" data-index="{{ $index }}">
            ...
            <div class="flex flex-col bg-white rounded-3xl px-8 py-4 border-4 border-customPink">
                <div class="flex items-center justify-between">
                    ...
                    <div class="text-sm text-gray-500 flex items-center font-bold">
                        ...
                        〜{{ $survey && $survey->end_date ? $survey->end_date->format('Y/m/d') : '期限なし' }}
                    </div>
                </div>
                            ...
                            @foreach (['5' => 'customBlue', '4' => 'customLightBlue', '3' => 'customGray2', '2' => 'customLightPink2', '1' => 'customLightPink'] as $key => $color)
                                @if ($questionAnswers[$question->id][$key] > 0)
                                    <div class="bg-{{ $color }} h-full flex items-center justify-center"
                                         data-width="{{ $questionAnswers[$question->id][$key] }}">
            ...
            <div class="flex flex-col bg-white rounded-3xl px-8 py-4 border-4 border-customPink">
                <div class="flex items-center justify-between">
                    <div>
                        <div class="flex items-center mb-4">
                            ...
                            <h1 class="text-3xl font-bold text-customPink">パルスサーベイ サマリー</h1>
                        </div>
                    </div>
                    <div class="text-sm text-gray-500 flex items-center font-bold">
                        <img src="{{ asset('image/calendar.png') }}" alt="Calendar" class="w-6 h-6 mr-2">
                ...
                <div class="w-full h-[1px] bg-customGray2 mx-auto my-2"></div>
                <button class="w-[280px] h-[53px] bg-customPink rounded-xl flex items-center justify-center mx-auto shadow-md font-semibold text-xl text-white keeping-btn" data-url="{{ route('keeping', ['dept_id' => $currentDeptId]) }}">
                    施策の継続
                </button>
                ...
                <div class="flex">
                    ...
                    <button class="w-1/3 h-[43px] bg-customLightPink2 rounded-lg flex items-center justify-center mx-auto shadow-md hover:bg-customLightPink planning-btn" data-url="{{ route('planning', ['dept_id' => $currentDeptId]) }}">
                        <span class="text-[16px] font-semibold text-customWhite">
                            施策・目標の変更
                        </span>
                    </button>
            ...
            <div class="flex bg-white rounded-3xl px-8 py-4 border-4 border-customPink">
                <div class="flex flex-col w-4/5 text-customNavy">
                    <div class="flex items-center">
                        <button id="accordionToggle2" class="relative group mr-2">
                            <svg id="accordionIcon2" class="w-8 h-8 text-[#82868B] transition-transform duration-500 rotate-90" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 16 16">
                                <path d="M6 12l4-4-4-4" stroke-linecap="round" stroke-linejoin="round"/>
                            </svg>
                            <span class="absolute top-[20px] left-1/2 -translate-x-1/2 w-[1.5px] h-[8px] bg-white transition-all duration-500"></span>
                        </button>
                        <h3 class="text-3xl font-bold">施策・短期目標の再設定</h3>
                    </div>
                    <div id="accordionContent2" class="transition-all duration-500 overflow-hidden">
                        <div class="border-b border-gray-300 my-1"></div>
                        <div class="flex items-center px-2">
                            <img src="./image/memo.png" alt="" class="mr-2">
                            <textarea id="auto-resize-textarea" class="border-none outline-none focus:outline-none focus:ring-0 w-full resize overflow-hidden h-auto" rows="1"></textarea>
                        ...
                        <div class="border-b border-gray-300 my-1"></div>
                        <button onclick="location.href='./planning'" class="bg-customPink w-full flex text-white px-4 py-6 rounded-xl mt-2">
                            <img src="./image/homeicon.png" alt="" class="mx-3 w-8">
                            <p class="font-bold text-2xl mr-auto">施策提案</p>
                            <p class="">>></p>
                        </button>
            ...
            <div class="flex flex-col bg-white rounded-3xl px-8 py-4 border-4 border-customBlue">
                <div class="flex items-center justify-between">
                    <div>
                        <div class="flex items-center">
                            <img src="{{ asset('image/BB_large.png') }}" alt="Icon" class="w-12 h-12 mr-2">
...
document.addEventListener('DOMContentLoaded', function() {
    // Planning and Keeping buttons
    document.querySelectorAll('.planning-btn, .keeping-btn').forEach(button => {
        button.addEventListener('click', function() {
